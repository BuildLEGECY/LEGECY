...
def check_rate_limit(request: Request) -> None:
    ...
    if len(_rate_limit_state) > 1000:
        ...
        for ip in [
            ip for ip, (started, _) in _rate_limit_state.items()
            if started < cutoff
        ]:
...
@app.get(
    ...
    description="Return process-local operational metrics for requests, wallet analysis, cache behavior, rate limiting, errors and response latency.",
...
@app.get(
    ...
    description=(
        ...
        "Deeper analysis uses the same bounded concurrent RPC pipeline and contributes to the "
        "same normalized activity, trading, behavior, reputation, smart-money and confidence profile."
    ),
...
async def analyze_wallet(
    ...
    limit: int = Query(
        ...
        description="Number of recent transactions to analyze. Default is 20; deeper analysis can request up to the configured maximum.",
    ),
):
    ...
    try:
        ...
        logger.warning(
            "Wallet analysis timed out for wallet=%s limit=%s timeout_seconds=%s",
            wallet_address,
            limit,
        ...
        raise HTTPException(
            status_code=504,
            detail={"message": "Wallet analysis timed out. Please try again later."},
        )
        ...
        raise HTTPException(
            status_code=500,
            detail={"message": "Wallet analysis failed. Please try again later."},
        )
