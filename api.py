...
class WalletComparisonResponse(BaseModel):
    wallet_a: Optional[str] = None
    wallet_b: Optional[str] = None
    winner: Dict[str, Any] = {}
...
app = FastAPI(
    ...
    description=(
        ...
        "data confidence, smart-money analysis, wallet comparison, smart-wallet "
        "discovery, smart-money ranking and watchlists."
    ),
    version=API_VERSION,
    docs_url="/docs",
    redoc_url="/redoc",
...
async def security_headers_middleware(request: Request, call_next):
    """Add browser and transport hardening headers without changing the dashboard CSP."""
...
async def request_logging_middleware(request: Request, call_next):
    ...
    try:
        ...
        _metrics["total_response_time_ms"] += duration
        ...
        _metrics["responses_5xx"] += 1
        logger.exception(
            "request_id=%s path=%s status=500 duration_ms=%.2f",
            rid,
            request.url.path,
            duration,
        )
...
def check_rate_limit(request: Request):
    ...
    started, count = _rate_limit_state.get(ip, (now, 0))
    ...
    if count >= RATE_LIMIT_REQUESTS:
        ...
        retry = max(
            1,
            int(RATE_LIMIT_WINDOW_SECONDS - (now - started)),
        )
        raise HTTPException(
            status_code=429,
            detail={
                "message": (
                    "Too many wallet analysis requests. "
    ...
    if len(_rate_limit_state) > 1000:
        ...
        for key in [
            k
            for k, (s, _) in _rate_limit_state.items()
            if s < cutoff
        ]:
            _rate_limit_state.pop(key, None)


def _cache_key(wallet: str, limit: int):
    return f"{wallet}:{limit}"


def get_cached_wallet_profile(
    wallet_address: str,
    ...
    if time.monotonic() - created >= CACHE_TTL_SECONDS:
        _wallet_cache.pop(
            _cache_key(wallet_address, history_limit),
            None,
        )
        return None
...
def cache_wallet_profile(
    ...
    if len(_wallet_cache) > CACHE_MAX_ENTRIES:
        _wallet_cache.pop(
            min(_wallet_cache, key=lambda k: _wallet_cache[k][0]),
            None,
        )
...
async def _analyze_for_compare(wallet: str, limit: int):
    ...
    profile = await build_wallet_profile(wallet, limit=limit)
    cache_wallet_profile(wallet, profile, limit)
    return profile
...
async def dashboard_enhancements():
    if not DASHBOARD_ENHANCEMENTS.exists():
        raise HTTPException(
            status_code=404,
            detail={"message": "Dashboard enhancements are not available."},
        )
    return FileResponse(
...
async def metrics():
    ...
    wr = _metrics["wallet_analysis_requests"]
    ...
    return {
        ...
        "requests": {
            ...
            "average_response_time_ms": round(
                _metrics["total_response_time_ms"] / total
                if total
                else 0,
                2,
            ),
        ...
        "wallet_analysis": {
            ...
            "average_time_ms": round(
                _metrics["wallet_analysis_time_ms"] / wr
                if wr
                else 0,
                2,
            ),
        ...
        "discovery": {
            "requests": _metrics["discovery_requests"],
            "success": _metrics["discovery_success"],
            "errors": _metrics["discovery_errors"],
        },
        "ranking": {
            "requests": _metrics["ranking_requests"],
            "success": _metrics["ranking_success"],
            "errors": _metrics["ranking_errors"],
        },
        "cache": {
